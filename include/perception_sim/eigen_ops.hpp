...
//
// `context` is an optional free-form tag (e.g. "cluster_2") used purely for
// CSV instrumentation (see csv_logger.hpp) - it has no effect on the result.
...
// matrix-vector multiply per point, done with Eigen. Each per-point multiply
// is logged to matrix_multiplications.csv via csv_logger.
