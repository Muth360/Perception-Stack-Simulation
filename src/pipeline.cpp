...
PipelineResult runPipeline(const CloudPtr& input_cloud, const PipelineConfig& config) {
    ...
    for (std::size_t i = 0; i < clusters.size(); ++i) {
        ...
        pca::ClusterPcaResult pca_result = pca::analyzeCluster(cluster, i);
        bbox::OrientedBoundingBox box = bbox::computeOrientedBoundingBox(cluster, pca_result, i);
        ...
        // Final, per-object cluster center as reported downstream (distinct
        // from the raw computeCentroid CSV rows, which fire for every
        // centroid calculation regardless of whether it becomes a
        // finalized object).
        CsvLogger::instance().logClusterCenter(obj.num_points, obj.pca_result.centroid,
